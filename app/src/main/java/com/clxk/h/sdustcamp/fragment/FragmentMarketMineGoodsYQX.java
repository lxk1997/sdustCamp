 ...
 * 二手市场-我的-闲置管理-已取消
 */
public class FragmentMarketMineGoodsYQX extends Fragment implements AdapterView.OnItemClickListener {
            ...
            public void done(List<MarketGoods> list, BmobException e) {
                if(e == null) {
                    ...
                    for(MarketGoods mg : list) {
                        if(mg.getgStatue() == 3 && mg.getUserId().equals(BmobUser.getCurrentUser(User.class).getUsername())) {
                            source.add(mg);
                        }
                    }
